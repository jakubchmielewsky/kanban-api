import { create, findAll, remove, update } from "../services/BoardService";
...
export const createBoard = catchAsync(async (req: Request, res: Response) => {
  ...
  const board = await create(teamId, name, req.user);
...
export const updateBoard = catchAsync(async (req: Request, res: Response) => {
  ...
  const board = await update(boardId, name, req.user);
...
export const deleteBoard = catchAsync(async (req: Request, res: Response) => {
  ...
  await remove(boardId, teamId, req.user);
  res.status(204).send();
});
